...
BEGIN
	IF (SELECT true FROM member WHERE login = p_login AND pass = p_pass) THEN
		BEGIN
			...
			SELECT id, nick, sex, privilege, first_login FROM member WHERE login = p_login AND pass = p_pass;
		END;
    END IF;
END$$
...
BEGIN
	SELECT id, login, nick, sex, privilege, first_login FROM member WHERE token = p_token;
END$$
